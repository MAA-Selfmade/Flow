...
class Component extends DCLogic {
  ...
  toggleTimer(id) {
    ...
    if (s.timerTask === id) {
      ...
      const r = this.closeTimer(Date.now());
      const tasks = r.tasks.map(t => t.id === id && (t.status === 'pipeline' || t.status === 'ide') ? Object.assign({}, t, { status: 'gang' }) : t);
  ...
  toggleLabelOn(id, lid) {
    const t = this.T(id), on = (t.labels || []).indexOf(lid) >= 0;
    this.patch(id, { labels: on ? t.labels.filter(x => x !== lid) : (t.labels || []).concat([lid]) });
  }
  initialsFor(name) {
  ...
  promoteItem(id, i) {
    ...
    const task = { id: nid, title: it.text, desc: '', project: t.project, owner: it.by, members: [it.by],
      labels: (t.labels || []).slice(), status: 'pipeline', prio: t.prio, start: null, due: it.due || t.due,
  ...
  createBatch() {
    ...
    const tasks = rows.map((r, i) => ({ id: 'b' + now + i, title: r.title, desc: '', project: r.project, owner: r.owner,
      members: [r.owner].concat(sh.members.filter(m => m !== r.owner)), labels: [], status: 'pipeline', prio: r.prio,
    ...
    this.setState({ tasks: this.state.tasks.concat(tasks), batch: this.blankBatch(), view: 'tabel',
      statusOff: Object.keys(this.state.statusOff).reduce((o,k) => { if (k !== 'pipeline') o[k] = this.state.statusOff[k]; return o; }, {}) });
    this.flash(tasks.length + ' opgaver oprettet i Pipeline.');
  }
  ...
  createFromTemplates() {
    ...
    const tasks = picks.map((tp, i) => {
      ...
      return { id: 'tp' + now + i, title: tp.name, desc: '', project: tp.project, owner,
        members: [owner].concat(sh.members.filter(m => m !== owner)), labels: [], status: 'pipeline',
  ...
  openNew() {
    ...
    this.setState({ draft: { editId: null, title: '', project: 'p1', owner: me, members: [me], labels: [], prio: 'normal',
      start: '', due: '', est: '', desc: '', checklist: [], files: [], status: 'pipeline',
  ...
  openEdit(id) {
    ...
    this.setState({ draft: { editId: id, title: t.title, project: t.project, owner: t.owner, members: t.members.slice(),
      ...
      recur: t.recur ? Object.assign({}, t.recur) : this.blankRecur(), newItem: { text: '', by: t.owner, due: '' }, showRecur: !!t.recur, private: !!t.private } });
  }
  applyTemplate(id) {
  ...
  L(id) { return this.state.labelDefs.filter(x => x.id === id)[0]; }
  lbVals(isOn, toggle) {
    const s = this.state;
    return {
        lbOpen: s.lbUI.open,
        ...
        lbRows: s.labelDefs.filter(l => !s.lbUI.q || l.name.toLowerCase().indexOf(s.lbUI.q.toLowerCase()) >= 0).map(l => {
          const on = isOn(l.id);
        ...
        lbEditHeading: s.lbUI.editId ? 'Rediger label' : 'Ny label',
  ...
  saveLabel() {
    ...
    if (!name) { this.flash('Giv labelen et navn.'); return; }
    if (u.editId) {
      this.setState({ labelDefs: this.state.labelDefs.map(l => l.id === u.editId ? Object.assign({}, l, { name, color: u.color }) : l) });
    } else {
      const id = 'l' + Date.now();
      this.setState({ labelDefs: this.state.labelDefs.concat([{ id, name, color: u.color }]) });
      if (this.state.draft) this.dset({ labels: this.state.draft.labels.concat([id]) });
  ...
  delLabel(id) {
    this.setState({ labelDefs: this.state.labelDefs.filter(l => l.id !== id),
      tasks: this.state.tasks.map(t => t.labels.indexOf(id) >= 0 ? Object.assign({}, t, { labels: t.labels.filter(x => x !== id) }) : t) });
    if (this.state.draft) this.dset({ labels: this.state.draft.labels.filter(x => x !== id) });
  ...
  saveDraft() {
    ...
    const patch = { title: d.title.trim(), project: d.project, owner: d.owner, members: d.members.length ? d.members : [d.owner],
      labels: d.labels, prio: d.prio, start: d.start || null, due: d.due || null, est: parseFloat(String(d.est).replace(',', '.')) || 0,
      desc: d.desc, checklist: d.checklist, files: d.files, recur: d.recur.mode === 'none' ? null : d.recur, recurring: rec, private: !!d.private };
    if (d.editId) {
  ...
  searchVals() {
    ...
    return {
      ...
      rows: hits.map((h, i) => ({ title: h.t.title,
        ...
        status: this.archived(h.t) ? 'Arkiveret' : STATUS[h.t.status].label, color: h.p.color || '#C9C4BA',
  ...
  renderVals() {
    ...
    const pool0 = s0.tasks.filter(t => !this.hiddenFor(t) && (s0.view === 'tid' || !this.archived(t)) && (!s0.projFilter || t.project === s0.projFilter)
    ...
    const v = {
      ...
      navViews: [mk('tabel','Opgaver','☰'), mk('gantt','Tidslinje','▭'), mk('kal','Kalender','▦'), mk('board','Board','▤'), mk('tid','Tid','◷'), mk('dash','Overblik','◫')],
      projectNav: s0.projects.map(p => ({ name: p.name, color: p.color,
        open: s0.tasks.filter(t => t.project === p.id && t.status !== 'faerdig' && !this.hiddenFor(t)).length,
      ...
      isCal: s.view === 'kal', isTable: s.view === 'tabel', isGantt: s.view === 'gantt', isTime: s.view === 'tid', isDash: s.view === 'dash',
      toast: s.toast
    };
    const titles = {
    ...
    const todayH = hOn(TODAY, s.me) + (s.timerTask && this.T(s.timerTask).owner === s.me ? running : 0);
    v.todayHours = hrs(todayH); v.todayPct = Math.min(100, todayH / 7.5 * 100) + '%'; v.todayRemain = hrs(Math.max(0, 7.5 - todayH));
    v.weekBars = week.map((dt, i) => { const h = hOn(dt, s.me) + (dt === TODAY && s.timerTask && this.T(s.timerTask).owner === s.me ? running : 0);
    ...
    if (s.view === 'kal') {
      ...
      if (showItems) s0.tasks.forEach(t => { if (this.hiddenFor(t) || this.archived(t) || (s0.projFilter && t.project !== s0.projFilter)) return;
    ...
    const varSrc = s.tasks.filter(t => this.liveSpent(t) > 0 && (s.timeScope === 'me' ? t.owner === s.me : true));
    ...
    if (s.sel && this.T(s.sel)) {
      ...
      v.d = Object.assign({}, c, {
        ...
        ...this.lbVals(id => (t.labels || []).indexOf(id) >= 0, id => this.toggleLabelOn(t.id, id)),
        labelToggles: s0.labelDefs.map(l => { const on = (t.labels || []).indexOf(l.id) >= 0;
          return { name: l.name, dot: l.color, bg: on ? l.color + '22' : '#fff',
            border: on ? l.color : '#DFDBD3', weight: on ? 600 : 400,
    ...
    if (dr) {
      ...
      v.m = {
        ...
        labelOpts: s.labelDefs.map(l => { const on = dr.labels.indexOf(l.id) >= 0;
          return { name: l.name, dot: l.color, bg: on ? l.color + '22' : '#fff',
            border: on ? l.color : '#DFDBD3', color: on ? '#231F20' : '#6E675F', weight: on ? 600 : 400,
            onClick: () => this.toggleIn('labels', l.id) }; }),
        ...this.lbVals(id => dr.labels.indexOf(id) >= 0, id => this.toggleIn('labels', id)),
        prOpen: s.prUI.open,
