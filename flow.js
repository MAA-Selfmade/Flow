...
class Component extends DCLogic {
  ...
  releaseNext(id, tasksIn, nextOcc) {
    ...
    const notes = next.map((t, i) => ({ id: 'a' + Date.now() + i, to: t.owner, tag: 'KLAR TIL DIG', kind: 'handoff', task: t.id,
      ...
      body: this.U(this.T(id).owner).name + ' afsluttede “' + this.T(id).title + '”. Opgaven er ikke længere blokeret.',
  ...
  openEdit(id) {
    ...
    this.setState({ draft: { editId: id, title: t.title, project: t.project, owner: t.owner, members: t.members.slice(),
      ...
      recur: t.recur ? Object.assign({}, t.recur) : this.blankRecur(), newItem: { text: '', by: t.owner, due: '' }, showRecur: !!t.recur } });
  }
  applyTemplate(id) {
  ...
  delItem(i) { this.dset({ checklist: this.state.draft.checklist.filter((c, j) => j !== i) }); }
  ...
  saveDraft() {
    ...
    const patch = { title: d.title.trim(), project: d.project, owner: d.owner, members: d.members.length ? d.members : [d.owner],
      ...
      desc: d.desc, checklist: d.checklist, files: d.files, recur: d.recur.mode === 'none' ? null : d.recur, recurring: rec };
    if (d.editId) {
  ...
  searchVals() {
    ...
    let hits = this.state.tasks.map(t => {
  ...
  renderVals() {
    ...
    const pool0 = s0.tasks.filter(t => (s0.view === 'tid' || !this.archived(t)) && (!s0.projFilter || t.project === s0.projFilter)
    ...
    const v = {
      ...
      navViews: [mk('tabel','Opgaver','☰'), mk('gantt','Tidslinje','▭'), mk('board','Board','▤'), mk('tid','Tid','◷'), mk('dash','Overblik','◫')],
      projectNav: s0.projects.map(p => ({ name: p.name, color: p.color,
        open: s0.tasks.filter(t => t.project === p.id && t.status !== 'faerdig').length,
      ...
      isTable: s.view === 'tabel', isGantt: s.view === 'gantt', isTime: s.view === 'tid', isDash: s.view === 'dash',
      toast: s.toast
    };
    const titles = {
    ...
    const keys = []; scoped.forEach(e => { const k = e[0] + '|' + e[1]; if (keys.indexOf(k) < 0) keys.push(k); });
    v.timeRows = keys.map(k => { const a = k.split('|'), t = this.T(a[0]), p = this.P(t.project);
      const cells = week.map(dt => { const h = scoped.filter(e => e[0] === a[0] && e[1] === a[1] && e[2] === dt).reduce((x, e) => x + e[3], 0);
        return { v: h ? num(h) : '–', color: h ? '#231F20' : '#C9C4BA' }; });
      const tot = scoped.filter(e => e[0] === a[0] && e[1] === a[1]).reduce((x, e) => x + e[3], 0);
    ...
    if (s.sel && this.T(s.sel)) {
      ...
      v.d = Object.assign({}, c, {
        ...
        showMembers: (t.members || []).filter(m => m !== t.owner).length > 0 || s0.memberPicker === t.id,
        showAddMember: (t.members || []).filter(m => m !== t.owner).length === 0 && s0.memberPicker !== t.id,
...
(function () {
  ...
  async function start(store, user, acc, tries) {
    ...
    let dirty = new Set(), timer = null, applying = false;
    function flush() {
      timer = null;
      ...
      dirty = new Set();
      if (ops.length) store.write(ops).catch((e) => { console.error(e); logic.flash('Kunne ikke gemme — tjek forbindelsen. Ændringen prøves igen.'); });
